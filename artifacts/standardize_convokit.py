...
def normalize(corpus: str, cid: str, rows: list[dict], source_meta: dict,
    ...
    for r in rows:
        ...
        parsed = meta.pop('parsed', None) if corpus == 'conversations-gone-awry-cmv-corpus' else None
...
def validate_paths(sources: list[Path], output: Path, sample_dir: Path) -> None:
    """Reject unsafe destinations before creating directories or writing files."""
    if output.resolve() == sample_dir.resolve():
        raise ValueError('output and sample directories must be different')
...
def validate_retention(totals: Counter) -> None:
    """Integrity gates must remain enabled under Python optimization (-O)."""
    if totals['input_utterances'] != totals['source_utterances'] + totals['rejected_utterances']:
        raise ValueError('source utterance retention mismatch')
    if totals['input_conversations'] != totals['output_conversations'] + totals['rejected_conversations']:
        raise ValueError('source conversation retention mismatch')
...
def run(source: Path, output: Path, sample_dir: Path):
    ...
    with tempfile.TemporaryDirectory(prefix='convokit-standardize-') as tmp:
        ...
        db.commit()
        row_cids = {r[0] for r in db.execute('SELECT DISTINCT cid FROM rows')}
...
def main():
    ...
    args = ap.parse_args()
    try:
        validate_paths([args.source_root / corpus for corpus in CORPORA], args.output_dir, args.sample_dir)
