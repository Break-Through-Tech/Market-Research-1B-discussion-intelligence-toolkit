...
folder for a separately published version. Source files remain read-only. Output
and sample directories must resolve to different locations (including symlink
aliases); both are checked against every source folder before writing.
...
- Multiple roots, cycles, invalid references/types and other invalid conversations
  ...
  the import rather than silently overwriting records. Metadata-only conversations
  with no messages are counted and quarantined rather than silently omitted.
  Retention checks raise explicit exceptions and remain active under `python -O`.
  This run rejected none.
...
source/output checksums, preservation of text, null flags, timestamps, voting
values, message and speaker metadata, conversation metadata/labels, vectors,
existing parent edges, and complete record retention. The audit checks graph
structure independently of the processing script's traversal validator.
`source-preservation-check.json` records the full audit result.

Nineteen automated tests (passing in normal and optimized Python) cover missing parents, shared placeholders, cycles,
...
the actual disk pipeline's quarantine/streaming behavior, output-directory aliases,
optimized-mode integrity checks, metadata-only conversations, corpus-specific
annotation retention, and detection of deliberately altered metadata and scores.
